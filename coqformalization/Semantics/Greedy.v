...
Definition greedy_complete e s :=
  unproblematic e -> s <<- e -> forall s', exists n o, greedy e (s ++ s') n o /\ o <> Error.

Definition greedy_complete_F
  : forall p, (forall p', input_lt p' p -> greedy_complete (get_regex p') (get_string p')) ->
         greedy_complete (get_regex p) (get_string p).
  intros [e s] IH Hue Hse s'.
  destruct e ; simpl in *.
  -
    inverts* Hse.
  -
    inverts* Hse.
    eexists ; eexists ; esplit ; eauto ; congruence.
  -
    inverts* Hse.
    exists 2 (Ok (String a "") s' []) ; split. constructor. congruence.
  -
    inverts* Hse.
    inverts Hue.
    assert (HLt1 : input_lt (mk_input e1 s0)
                            (mk_input (e1 @ e2) (s0 ++ s'0)))
      by (apply left_lex ; unfolds ; crush).

    assert (HLt2 : input_lt (mk_input e2 s'0)
                            (mk_input (e1 @ e2) (s0 ++ s'0)))
      by (apply left_lex ; unfolds ; crush).
    lets IH1 : IH HLt1 H1 H2 (s'0 ++ s').
    lets IH2 : IH HLt2 H4 H3 s'.
    destruct IH1 as [n1 [o1 [Hg1 Heq1]]].
    destruct IH2 as [n2 [o2 [Hg2 Heq2]]].
    destruct o1 ; crush.
    destruct o2 ; crush.